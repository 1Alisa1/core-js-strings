...
function sumOfCodes(str) {
  ...
  if (!str || !str.length || str.length < 1) return 0;
...
function isPalindrome(str) {
  const temp = str.toLowerCase().replace(/[^a-zA-Z]/g, '');

  const str1 = temp.split('').reverse().join('');
...
function findLongestWord(sentence) {
  const longestWord = sentence.split(' ').sort((a, b) => b.length - a.length);
...
function encodeToRot13(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    encoded +=
      input.indexOf(str[i]) === -1 ? str[i] : output[input.indexOf(str[i])];
  }
